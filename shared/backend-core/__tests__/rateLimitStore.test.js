...
test("no REDIS_URL: returns undefined so express-rate-limit uses its own default per-process store", () => {
  ...
  const storePath = require.resolve("../rateLimitStore");
...
test("all four express-rate-limit instances are wired through createRateLimitStore (index.js x3, upload.routes.js x1)", () => {
  const fs = require("fs");
  const indexSrc = fs.readFileSync(require.resolve("../../../backend/src/index.js"), "utf8");
  const uploadSrc = fs.readFileSync(require.resolve("../../../features/storage-upload/backend/upload.routes.js"), "utf8");
  assert.match(indexSrc, /require\(".*shared\/backend-core\/rateLimitStore"\)/);
