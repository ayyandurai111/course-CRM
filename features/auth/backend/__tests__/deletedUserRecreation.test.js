...
function loadServiceWithFakes({ updateResult = { error: null }, authDeleteResult, pendingUsers = [] }) {
  ...
  const authAdminPath = require.resolve("../authAdmin.lib");
  const servicePath = require.resolve("../../../../features/storage-upload/backend/userDeletionService");
  ...
  const service = require("../../../storage-upload/backend/userDeletionService");
...
test("deleteAuthUserSafely treats a 'not found' Auth error as success (idempotent retry)", async () => {
  ...
  const { deleteAuthUserSafely } = require("../authAdmin.lib");
