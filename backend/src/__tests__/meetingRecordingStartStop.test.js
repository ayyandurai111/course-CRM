...
function freshRecordingConfig() {
  delete require.cache[require.resolve("../lib/recordingConfig")];
  return require("../lib/recordingConfig");
}
function freshRecordingService() {
  delete require.cache[require.resolve("../services/meetingRecordingService")];
  delete require.cache[require.resolve("../lib/recordingConfig")];
  return require("../services/meetingRecordingService");
}
...
test("startRecording: success — writes to the canonical courses/{courseId}/videos/{contentId}/ path and returns a RECORDING patch", async () => {
  await withEnv(S3_ENV, async () => {
    const { startRecording } = freshRecordingService();
    const { parseStoragePath } = require("../lib/fileValidation");
