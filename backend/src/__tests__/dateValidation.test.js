...
const { isValidDate, parseValidDate, isoDateTimeString, futureIsoDateTimeString } = require("../lib/dateValidation");
...
const { scheduleBodySchema } = require("../routes/content.routes");
...
test("contentService.schedule/reschedule use parseValidDate — the unsafe raw `new Date(x) <= new Date()` comparison is gone", () => {
  const fs = require("fs");
  const src = fs.readFileSync(require.resolve("../services/contentService"), "utf8");
